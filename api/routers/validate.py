...
# Cap how many issues we return so a wildly invalid document can't produce a huge payload.
MAX_ERRORS = 50


def _validate_bytes(xml_bytes: bytes, xsd_path: Path) -> dict:
    """Validate XML bytes against an XSD file; return a JSON-serializable result.

    Reuses tools.validate_xml.validate_xml (lxml) by writing the XML to a temp file, so the
    API and CLI share the same validation engine. Malformed XML is reported as an invalid
    result (not a 500) so the UI can surface the parse error inline.
    """
...
# NOTE: declare /upload before /{schema_name} so the literal route is matched first
# (Starlette resolves routes in declaration order; otherwise "upload" is read as a schema name).
@router.post("/upload")
async def validate_upload(
    ...
    """Validate a user-uploaded canonical XML against a user-uploaded XSD (custom-schema flow)."""
