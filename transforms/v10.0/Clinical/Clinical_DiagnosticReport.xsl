<xsl:stylesheet xpath-default-namespace="http://cocodata.org" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <DiagnosticReport xmlns="http://hl7.org/fhir">
                    ...
                    <code>
                        <coding>
                            ...
                            <!-- in examples but not source. <display></display> -->
                        </coding>
                    </code>
                    <subject>
                        <reference>
                    ...
                    <effectiveDateTime>
                        <xsl:attribute name="value">
                            <xsl:value-of select="$DxReportLab/effective/effective_date_time/start"/>
                        </xsl:attribute>
                    </effectiveDateTime>
                    <issued>
                        <xsl:attribute name="value">
                            ...
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <reference>
                                        <xsl:attribute name="value">
                                            <xsl:value-of
                                                select="concat('#', ./reported_organization/name[1],'-','performerOrganizationDerived1' )"/>
                                        </xsl:attribute>
                                    </reference>
                                    <display>
                                        <xsl:attribute name="value">
                    ...
                    <xsl:for-each select="./result">
                        <result>
                            ...
                            <display>
                                <!-- Might want observation_value but it's blank in source. -->
                                <xsl:attribute name="value">
                                    <xsl:value-of select="./observation_value/display"/>
                                </xsl:attribute>
                            </display>
                        </result>
                    </xsl:for-each>
    ...
    <xsl:template name="Internal_results_container">
        ...
        <xsl:for-each select="./performer">
            ...
            <xsl:choose>
                <xsl:when test="exists(./reported_practitioner)">
                    <Practitioner>
                        ...
                        <identifier>
                            ...
                            <value>
                                <xsl:attribute name="value">
                                    <xsl:value-of select="./reported_practitioner/npi"/>
                                </xsl:attribute>
                            </value>
                        </identifier>
    ...
    <xsl:template name="Internal_participant_container">
        ...
        <xsl:for-each select="./performer">
            <xsl:if test="exists(./reported_practitioner)">
                    <Practitioner>
                        <!-- <xsl:call-template name="resource_meta"/> James:This node is not in sample output...-->
                        <id>
                            <xsl:attribute name="value">
                        ...
                        <!-- 0..* Identifier An identifier for the person as this agent -->
                        <xsl:if test="exists(./reported_practitioner/npi)">
                            <identifier>
                                <system value="http://hl7.org/fhir/sid/us-npi"/>
                                <value>
                                    <xsl:attribute name="value">
