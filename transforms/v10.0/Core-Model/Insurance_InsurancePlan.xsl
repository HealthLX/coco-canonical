...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="PTT_PAYER_DEMOGRAPHICS" select="insurance_plan"/>
    
    <xsl:variable name="PTT_CUSTOMER_PREFIX" select="$PTT_PAYER_DEMOGRAPHICS/customername"/>
    ...
    <InsurancePlan xmlns="http://hl7.org/fhir"> 
        ...
        <id>
            <xsl:attribute name="value">
                <xsl:value-of
                    select="concat($PTT_CUSTOMER_PREFIX, '-', $PTT_PAYER_DEMOGRAPHICS/plan_id/value)"
                />
            </xsl:attribute>
        </id>
        <meta>
            <xsl:variable name="EOB_PARENTFILE_NAME"
                select="$PTT_PAYER_DEMOGRAPHICS/parentfile"/>
            <source>
                <xsl:attribute name="value">
        ...
        <identifier>
            <system>
                <xsl:attribute name="value">
                    <xsl:value-of select="'http://cms.gov/medicare/ma-plan-id'"/>
                </xsl:attribute>
            </system>
            <value>
                <xsl:attribute name="value">
                    <xsl:value-of
                        select="$PTT_PAYER_DEMOGRAPHICS/plan_id/value"/>
                </xsl:attribute>
            </value>
        </identifier>
        <!-- Organization identifier -->
        <xsl:if test="$PTT_PAYER_DEMOGRAPHICS/system and $PTT_PAYER_DEMOGRAPHICS/system != 'http://cms.gov/medicare/ma-plan-id'">
            <identifier>
                <sytem>
                    <xsl:attribute name="value">
                        <xsl:value-of select="$PTT_PAYER_DEMOGRAPHICS/system"/>
                    </xsl:attribute>
                </sytem>
                <value>
                    <xsl:attribute name="value">
        ...
        <!-- 0..* Business Identifier for Product. Internal
            <identifier>
            <system>                
                <xsl:attribute name="value">
                    <xsl:value-of select="concat('https://data.healthlx.com/', $PTT_CUSTOMER_PREFIX,'-InsurancePlanID')"/>
                </xsl:attribute>
            </system>
            <value>
                <xsl:attribute name="value">
        ...
        <name>
                ...
                <xsl:attribute name="value">
                    <xsl:value-of
                        select="$PTT_PAYER_DEMOGRAPHICS/name"/>
                </xsl:attribute>
      ...
      <!--  <alias value="[string]"/><!-\- 0..* Alternate names -\->-->
        <period>
            <start>
                <xsl:attribute name="value">
                    <xsl:value-of select="concat(substring(./datetimereported, 1, 4), '-01-01')"/>
                </xsl:attribute>
            </start>
            <end>
                <xsl:attribute name="value">
                    <xsl:value-of select="concat(substring(./datetimereported, 1, 4), '-12-31')"/>
                </xsl:attribute>
            </end>
        </period>
        ...
        <administeredBy>
            <value>
                <xsl:attribute name="value">
                    <xsl:value-of
                        select="$PTT_PAYER_DEMOGRAPHICS/administered_by/name"/>
                </xsl:attribute>
            </value>
        ...
        <endpoint><!-- 0..* Reference(Endpoint) Technical endpoint --></endpoint>
         <xsl:for-each select="$PTT_PAYER_DEMOGRAPHICS/networks/network">
        <network><!-- 0..* Reference(Organization) What networks are Included -->
            <reference>
                <xsl:attribute name="value">
                    <xsl:value-of
                        select="concat('Organization/' ,$PTT_CUSTOMER_PREFIX ,'-', ./network_id)"/>
