...
<xsl:stylesheet version="1.0" xmlns="http://hl7.org/fhir"
    ...
    <xsl:template match="/">
        ...
        <Observation xmlns="http://hl7.org/fhir">
           ...
           <identifier>
                <value>
                    <xsl:attribute name="value">
                        <xsl:value-of select="concat($PTT_CUSTOMER_PREFIX, '-', $PTT_MEMBER_DEMOGRAPHICS/member_id, '-smoke')"/>
                    </xsl:attribute>
                </value>
            </identifier>
            <status value="final"/>
  ...
  -->
            <effectiveDateTime>
                <xsl:choose>
                    <xsl:when test="normalize-space($PTT_MEMBER_DEMOGRAPHICS/smoking_effective_date_time)">
                        <xsl:attribute name="value">
                            <xsl:value-of select="normalize-space($PTT_MEMBER_DEMOGRAPHICS/smoking_effective_date_time)"/>
                        </xsl:attribute>
                    </xsl:when>
                    <xsl:otherwise>
