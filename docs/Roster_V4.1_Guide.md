...
# Roster_V4.1 Implementation Guide

**HLX0123 HLX Roster_V4.1 IG (XSD_V4.1)**
...
7. [Required Elements of Roster_V4.1 XSD](#required-elements-of-roster_v4.1-xsd)
8. [All Elements of Roster_V4.1 XSD](#all-elements-of-roster_v4.1-xsd)
...
This implementation guide provides field mappings and requirements for HealthLX Roster_V4.1 data submissions in XML format based on FHIR R4 standards. XML format enables structured data exchange with built-in validation against the provided XSD schema.
...
<h2 style="color:#E60073">Required Elements of Roster_V4.1 XSD</h2>
...
| – | member | – | All of (any order): us_core_race, us_core_ethnicity, us_core_birth_sex, is_subscriber, relationship, birth_date, deceased_date_time, gender, unique_person_id, unique_person_id_assigner, unique_person_id_assigner_type, member_identity, member_id, member_id_system, subscriber_id, names, telecoms, addresses, health_coverage, communications, smoking_status, record_type, unique_record_identifier | – | all |
...
| use | name | 1..1 | Use this element to describe the name. More information can be found here: http://hl7.org/fhir/R4/valueset-name-use.html | – | – |
...
| period | address | 1..1 | Time period when this address was/is in use. If the address is still in use, do not supply an End date. Format is YYYY-MM-DD. | – | Period |
| health_coverage | member | 1..1 | – | – | – |
| group_number | health_coverage | 1..1 | – | – | string |
| policy_number | health_coverage | 1..1 | Each person covered by a health insurance plan has a unique ID number that allows healthcare providers and their staff to verify coverage and arrange payment for services. This is also known as member number and/or card-id and or member-id. | – | string |
...
| smoking_status | member | 1..1 | This element is for selecting the current smoking status of the member (449868002 = Current every day smoker, 428041000124106 = Current some day smoker, 8517006 = Former smoker, 266919005 = Never smoker, 77176002 = Smoker - current status unknown, 266927001 = Unknown if ever smoked, 428071000124103 = Current Heavy tobacco smoker, 428061000124105 = Current Light tobacco smoker). More information can be found here: http://hl7.org/fhir/us/core/ValueSet-us-core-observation-smokingstatus.html | – | – |
| record_type | member | 1..1 | This element describes the action for this member (A = Add, U = Update, D = Delete) | – | – |
...
<h2 style="color:#E60073">All Elements of Roster_V4.1 XSD</h2>
...
| gender | member | 1..1 | Use this element for Gender (male, female, other or unknown) | – | – |
| unique_person_id | member | 0..1 | This is the person's unique member number in the Payer system across plans. This number is not reused for anyone else. | – | string |
| unique_person_id_assigner | member | 0..1 | Organization that issued id | – | string |
...
| telecoms | member | 0..1 | – | – | – |
| telecom | telecoms | 0..unbounded | Contact points of telecommunications. Please provide at least one form of contact (e.g. phone, email, etc.) | – | – |
| system | telecom | 1..1 | Use this element to descripbe the contact point. https://www.hl7.org/fhir/valueset-contact-point-system.html | – | – |
| value | telecom | 1..1 | The actual value of the contact point. This is a free form text field allowing country and extension. (e.g. (+001) 111-111-1111 x1111) | – | string |
...
Roster_V4.1 files should be submitted according to the schedule agreed upon with HealthLX. Typical submission frequencies include daily, weekly, or monthly updates.
